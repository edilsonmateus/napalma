...
const SLOT_CATALOG = [
  {
    ...
    area: "Explorar",
    touchpoint: "Cartaz patrocinado no feed",
    ratio: "4 / 5",
    aspectRatio: "4:5",
    width: 1080,
    height: 1350,
...
function formatPatacos(value) {
  return new Intl.NumberFormat("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 3 }).format(Number(value || 0));
}
function campaignRemainingPatacos(campaign) {
  return Number(campaign?.metrics?.remainingPatacos ?? (Number(campaign?.reservedMilipatacos || 0) / 1000));
}
...
function campaignState(campaign) {
  ...
  const hasBudget = campaignRemainingPatacos(campaign) > 0;
...
function campaignReadyToActivate(campaign) {
  return campaign.status !== "active"
    ...
    && campaignRemainingPatacos(campaign) > 0
...
export default function AdvertiserPortalPage() {
  ...
  useEffect(() => {
    ...
    if (status) {
      const describeApprovedPayment = async () => {
        ...
        const linkedCredits = campaignRemainingPatacos(campaign);
  ...
  async function continueFromBudget() {
    ...
    if (campaignRemainingPatacos(campaign) > 0) return goWizardStep("review", campaign.id);
    ...
    setBusy(true);
    try {
      await allocateMyWalletCredits(accountId, { campaignId: campaign.id, amountMilipatacos: Number(wallet.balanceMilipatacos || 0) });

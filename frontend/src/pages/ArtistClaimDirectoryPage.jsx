...
export default function ArtistClaimDirectoryPage() {
  ...
  return <section className="screen artist-directory-screen">
    ...
    {showInclusion ? <div className="modal-backdrop claim-form-backdrop"><form className="modal-card claim-form artist-inclusion-form" onSubmit={submitInclusion}><h3>Solicitar inclusão de artista</h3><p>Preencha os dados para análise da equipe 77Gira.</p><label>Nome artístico ou da banda<input required minLength={2} placeholder="Como o artista é conhecido" value={inclusion.artistName} onChange={(event) => setInclusion({ ...inclusion, artistName: event.target.value })}/></label><div className="artist-inclusion-grid"><label>Cidade-base<input required placeholder="Ex.: São Paulo" value={inclusion.baseCity} onChange={(event) => setInclusion({ ...inclusion, baseCity: event.target.value })}/></label><label>Estado<input required maxLength={40} placeholder="Ex.: SP" value={inclusion.baseState} onChange={(event) => setInclusion({ ...inclusion, baseState: event.target.value })}/></label></div><label>Gêneros <span>(opcional)</span><input placeholder="Ex.: samba, pagode" value={inclusion.genres} onChange={(event) => setInclusion({ ...inclusion, genres: event.target.value })}/></label><label>Nome do responsável<input required placeholder="Nome completo" value={inclusion.responsibleName} onChange={(event) => setInclusion({ ...inclusion, responsibleName: event.target.value })}/></label><label>Telefone ou WhatsApp<input required inputMode="tel" placeholder="(00) 00000-0000" value={inclusion.responsiblePhone} onChange={(event) => setInclusion({ ...inclusion, responsiblePhone: event.target.value })}/></label><label>CPF ou CNPJ do solicitante<input required placeholder="Somente números" value={inclusion.claimantDocument} onChange={(event) => setInclusion({ ...inclusion, claimantDocument: event.target.value })}/></label><label>Seu vínculo com o artista<input required placeholder="Ex.: empresário ou gestor" value={inclusion.relationshipRole} onChange={(event) => setInclusion({ ...inclusion, relationshipRole: event.target.value })}/></label><label>Como podemos confirmar o vínculo e a existência do artista?<textarea required minLength={5} placeholder="Informe referências, canais oficiais ou outras evidências" value={inclusion.justification} onChange={(event) => setInclusion({ ...inclusion, justification: event.target.value })}/></label><div className="form-actions-inline"><button className="btn-primary" type="submit" disabled={createClaim.isPending}>{createClaim.isPending ? "Enviando..." : "Enviar para análise"}</button><button className="chip" type="button" onClick={() => setShowInclusion(false)}>Cancelar</button></div></form></div> : null}
  </section>;
}

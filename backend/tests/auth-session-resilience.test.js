...
describe("auth session resilience", () => {
  it("only treats confirmed authentication failures as definitive", () => {
    expect(isDefinitiveSessionFailure(failure(401, "invalid_refresh_token", "/auth/refresh"))).toBe(true);
    expect(isDefinitiveSessionFailure(failure(403, "forbidden"))).toBe(true);
  ...
  it("preserves the session for temporary infrastructure failures", () => {
    ...
    expect(isTransientSessionFailure(failure(401, "invalid_refresh_token"))).toBe(false);
  });
});
